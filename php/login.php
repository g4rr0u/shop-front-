...
$stmt_checkForData = "SELECT * FROM users WHERE user_login = ?";
...
if($stmt_checkForData){
    ...
    if($result->num_rows > 0){
        ...
        if(password_verify($user_password, $user_data['user_password'])){
            ...
            $user_fullname = $user_data['user_surname'] . " " . $user_data['user_name'] . " " . $user_data['user_patronymic'];
            $response = array('success' => true, 'user_id' => $user_id, 'user_role' => $user_role, 'user_fullname' => $user_fullname);
        } else {
